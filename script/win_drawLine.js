...
function txtMove(point){
	txtPoint=windowToCanvas(point.x,point.y);
	if(isOnLine({x:txtPoint.x,y:txtPoint.y})){//若移动的文本经过线条，则将该线条变为修改的状态
		modifyIndex=delIndex;
		delIndex=-1;
	}
	reDraw();
	drawTxtMove({x:txtPoint.x,y:txtPoint.y});
...
$("#scene").click(function(){
	...
	$("#spul").empty();
	str="<li><img src='../image/new.png' alt='from canvas'/></li>";
	$("#spul").append(str);
...
//将canvas 保存成图片
$("#saveToImage").click(function(){
	if(imgindex>=0){//将-1定位新增，>-1 则为打开已存在的图片，则修改数据库数据和图片//删除以前的，保存现在的，
...
//连接蓝牙设备
$("#linkble").click(function(){
	if(uuid=='' || cuuid==''){
		initmg();
	}else{
		setNo();
		//alert("click,uuid:"+uuid);
	}
});
...
hammertxt1.on("tap", function (e){
	linetxt=$("#blText1").val();
	if(delIndex!=-1){
		line[delIndex].len=linetxt;
		changeTopBar();
	}
//	panTxt=true;
...
hammertime.on("panmove", function (e) {
	...
	if(!isNewLine && isM==0){
		//alert("-canvasWidth:"+(-canvasWidth)+",(tsx+e.deltaX-mx):"+(tsx+e.deltaX-mx));
		if(-canvasWidth*(scaleNum-1)<=(tsx+e.deltaX-mx) &&  (tsx+e.deltaX-mx)<=0)
		{//控制 拖拽，以免超出画布范围
			tsx=tsx+e.deltaX-mx;//每次只需把拖动的增量 加到画布原点偏移量中，
		}
		if(-canvasHeight*(scaleNum-1)<=(tsy+e.deltaY-my) &&  (tsy+e.deltaY-my)<=0)
		{
			tsy=tsy+e.deltaY-my;
		}
		MoveCanvas();
...
$("#draw").click(function(){
	if(isNewLine==0){
		isNewLine=1;//重新画一条线
		$("#draw").removeClass("brush_p");
		$("#draw").addClass("brush_p_active");
	}else{
		isNewLine=0;//修改以前的线条
		$("#draw").removeClass("brush_p_active");
		$("#draw").addClass("brush_p");
	}
...
$("#listi").click(function(){
	...
	$("#name").val(line[delIndex].name);
	$("#len").val(line[delIndex].len);
	$("#angel").val(line[delIndex].angel);
	$("#des").val(line[delIndex].des);
});
...
//	    		}
	    	...
	    	if(!isM){//没有线条被修改，在移动不划线
	    		...
	    		hbackData(line[modifyIndex],modifyIndex,'m');
	    	}
    	}else{
    		isMouseDown=false;//移动不画线
    	}
...
function endStroke(point){
	isMouseDown=false;
	//alert("(endTime-startTime):"+(endTime-startTime));
	if((endTime-startTime)<200){//如果触摸开始与结束的时间差小于100则视为点击事件
		if(isNewLine){
...
//				drawMemLine();
		...
		if(delIndex==-1 && modifyIndex==-1 && isM==0){//如果单击画布没有改变线条状态，则不需要刷新画布
		}else{
			modifyIndex=-1; //此时修改结束，不需要将修改的线条画成红色
...
function moveStroke(point){
	...
	if(isNewLine){//是否 画新的线条
		...
		if(changePoint==0){//起点被移动
			line[modifyIndex].startPoint.x=(curLoc.x+(-tsx))/scaleNum;
			line[modifyIndex].startPoint.y=(curLoc.y+(-tsy))/scaleNum;
		}else{//终点被移动
			line[modifyIndex].endPoint.x=(curLoc.x+(-tsx))/scaleNum;
			line[modifyIndex].endPoint.y=(curLoc.y+(-tsy))/scaleNum;
		}
	}
	drawMemLine();
...
function closeModal(){
	...
	$(".del").removeClass("active");
	$("#listMode").addClass("hidden");
	$("#swip").addClass("hidden");
	$("#orderDiv").addClass("hidden");
	$("#bgpicDiv").addClass("hidden");
}
...
//写文本
function drawText(){
	context.font = "13px  Arial";            //设置文本大小 + 字体
	...
	for(var i=0;i<line.length;i++){
		...
		if((line[i].len+line[i].angel)){//如果长度和角度为空则不用再画布上画出来
			...
			if(y1==y2){
				...
				context.fillText(line[i].len+(line[i].angel?','+line[i].angel:''),recx+w/2,recy-5);
			}else{
				//context.fillText(line[i].len+','+(line[i].angel || 0),w/2,-5);
				context.fillText(line[i].len+(line[i].angel?','+line[i].angel:''),w/2,-5);
			}
		}
...
function reDraw(){//画布重画
	...
	canvasWidth=$("#main").width()*scaleNum;
	canvasHeight=$("#main").height()*scaleNum;
	if(isbgpic==0){//没有底图
		drawGrid();
	}else{//需要画底图
		drawBgpic();
...
function drawMemLine(){
	...
	for(var i=0;i<line.length;i++){
		...
		if(i==modifyIndex){//若不为新画线，当有线条被修改，需要重新画线时，被修改的线条颜色为红色，其余的为黑色
			context.strokeStyle=moveStrokeColor;
			context.setLineDash([]);
		}
...
function drawarc(){
	...
	for(var i=0;i<line.length;i++){
		r1=r2=8;
		if(modifyIndex==i){//若有线条将被修改，则端点圆圈变大
			if(changePoint==0){
				r1=20;
			}else{
				r2=20;
...
function drawGrid(){
	...
	context.save(); 
	context.translate(0.2,0.2); //中心偏移法
	context.beginPath();
...
function drawBgpic(){
	//imageOjb.onload=function(){//图片加载完成再画出来
		context.drawImage(imageOjb, 0, 0,canvasWidth,canvasHeight);
	//};
