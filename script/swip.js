...
//	3.然后将线条显示在画布上
	...
	getLines(canpicid,dbToLine);//从数据库获取线条，并将线条放入数组中
	
	getLines(canpicid,setBgpic);//获取背景图片
...
$("#listDel").click(function(){//删除该场景图片
	...
	var str="<li><img src='../image/new.png' alt='from canvas'/></li>";
	$.each(imgs,function(i,n){
		str=str+"<li><img src='"+n+"' alt='from canvas'/></li>";
	});
	$("#spul").empty().append(str);
	//删除 完成后需要隐藏命令菜单
	$("#orderDiv").toggleClass("hidden");
	
	var divWidth=$("#wrap").width()*0.9;//底部div的宽度
	var ulWidth=$("#spul").width();//滑动层的宽度
	if(ulWidth<=divWidth){//当可滑动的宽度小于外部包裹层宽度，则偏移量归0
		slsx=0;
		slider();
	}
});
...
/////////////////////更换场景图片 点击事件///////////////////
function setRedraw(reurl){//先判断是是新打开画布，还是重设底图，然后在判断是否设置了底图
	bgsrc=reurl;
	
	if(imgindex==-1)	{//新打开画布，设置底图
		line=[];
		if(bgsrc){
			isbgpic=1;
			imageOjb.src=bgsrc;
		...
		var canpicid=imgid[imgindex];
		getLines(canpicid,mdurlBycanpicid);
...
//		line=[];
//		reDraw();
	}else if(tempindex-1==imgindex){//如果点的是同一张图片，则显示或不显示////由于第一张图片没有存在数组中，所以要减一
		$("#orderDiv").toggleClass("hidden");
